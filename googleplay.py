    ...
    def executeRequestApi2(self, path, datapost=None, post_content_type="application/x-www-form-urlencoded; charset=UTF-8"):
        if (datapost is None and path in self.preFetch):
            ...
            headers = { "Accept-Language": self.lang,
                                    ...
                                    "User-Agent": "Android-Finsky/7.9.34.Q-all%20%5B0%5D%20%5BPR%5D%20159126641 (api=3,versionCode=80793400,sdk=22,device=hammerhead,hardware=hammerhead,product=hammerhead,platformVersionRelease=5.1,model=Nexus%205,buildId=LMY47D,isWideScreen=0,supportedAbis=armeabi-v7a;armeabi)",
...
#        print message.payload.buyResponse.purchaseStatusResponse.appDeliveryData
...
#                  }
        headers = {
                   "User-Agent" : "AndroidDownloadManager/5.1 (Linux; U; Android 5.1; Nexus 5 Build/LMY47D)",
    ...
    def download2(self, packageName, versionCode, file_name, offerType=1):
        """Download an app and return its raw data (APK file).
...
#                  }
        ...
        #progress = ProgressBar(file_name, total=content_size,unit="KB", chunk_size=chunk_size, run_status="downloading..", fin_status="Downloading completes")
        ...
        with open(file_name, "wb") as file:
            for data in response.iter_content(chunk_size=chunk_size):
               file.write(data)
               downlaod_count += len(data)
               p.update(downlaod_count * 100 / content_size)
            p.finish()
